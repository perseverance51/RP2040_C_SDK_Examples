...
//--------UI对象类型
DigitalPage *p_cur_ui ; // 当前操作的ui对象的指针(默认使用defaultui)
//main页面的图标
Icon main_icon_array[MAIN_PAGE_NUM] = {
    [0] = {
        0xFC, 0xFE, 0xFF, 0x3F, 0x1F, 0x0F, 0x07, 0x03, 0x03, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    ...
    [1] = {
        0xFC, 0xFE, 0x7F, 0x3F, 0x1F, 0x0F, 0x07, 0x03, 0x83, 0x81, 0x01, 0x01, 0x81, 0xE1, 0xE1, 0xE1,
    ...
    [2] = {
        0xFC, 0xFE, 0x07, 0xF3, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0xFB, 0x03, 0x03,
        ...
        0xF7, 0xF7, 0xF7, 0xF7, 0xF3, 0xF8, 0xDF, 0xCF
    }, //Curve
    [3] = {
        0xFC, 0xFE, 0x1F, 0x07, 0x07, 0xC3, 0xE3, 0xE3, 0xE3, 0xE3, 0xE3, 0xE3, 0xE3, 0x63, 0x63, 0x63,
        ...
        0xF1, 0xF1, 0xF0, 0xF8, 0xF8, 0xFE, 0xDF, 0xCF
    }, //DigitalLock
    [4] = {
        0xFC, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F, 0x3F, 0x3F, 0x1F, 0x0F, 0x0F, 0x0F, 0x07, 0x07, 0x07,
        ...
        0xF8, 0xF8, 0xF8, 0xF8, 0xFF, 0xFF, 0xDF, 0xCF
    } //about
};
...
const uint8_t logo_version_pic[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE0, 0xE0, 0xE0, 0xE0, 0xDC, 0x9C,
                                    ...
                                    0x02, 0x03, 0x03, 0x03, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
                                   }; //36*60 版
//镭射图片对象
...
Option setting_option_array[SETTING_PAGE_NUM] = {
    ...
    {0, 255, 0, 1, 0, "~ List Ani"}, //对于~,表示显示数值弹窗，需要给定最大最小值和步长
    {.text = "+ LED1 GPIO25", .step = 1,},//对于+,表示 ‘口’ 表示可以点击，点击后会触发click事件
    {.text = "+ LED2 GPIO6", .step = 1,},
...
uint8_t lock_password[3] = {[Digital_Pos_IndexLeft] = 11, [Digital_Pos_IndexMid] = 11, [Digital_Pos_IndexRight] = 11}; //3个两位数字组成的密码(注意三个数字从0-2是从右到左)
...
void MainPage_CallBack(uint8_t self_page_id, Option* select_item)
{
    switch(select_item->order) {
        case 0:
            OLED_UIJumpToPage(self_page_id, &logo_page);
            break;
        case 1:
            OLED_UIJumpToPage(self_page_id, &setting_page);

            break;
        case 2:
            OLED_UIJumpToPage(self_page_id, &curve_page);
            break;
        case 3:
            OLED_UIJumpToPage(self_page_id, &lock_page);
            break;
        case 4:
            OLED_UIJumpToPage(self_page_id, &about_page);
            break;
        default:
            break;
    }
...
void SettingPage_CallBack(uint8_t self_page_id, Option* select_item)
{
    switch(select_item->order) { //对选中项的真实参数值赋值
        //由于第0项是说明文字“Setting”
        case 1:
            ui_para.ani_param[TILE_ANI] = select_item->val;
            break;//ani_tile
        case 2:
            ui_para.ani_param[LIST_ANI] = select_item->val;
            break;//ani_list
        case 3:
            ui_para.ufd_param[TILE_UFD] = select_item->val;
            break;//ani_tile
        case 4:
            ui_para.ufd_param[LIST_UFD] = select_item->val;
            break;//ani_list
        case 5:
            ui_para.loop_param[TILE_UFD] = select_item->val;
            break;//loop_tile
        case 6:
            ui_para.loop_param[LIST_UFD] = select_item->val;
            break;//loop_list
        case 7:
            ui_para.valwin_broken = select_item->val;
            break;//ValWin Broken
        case 8:
            ui_para.conwin_broken = select_item->val;
            break;//ConWin Broken
        case 9:
            ui_para.digital_ripple = select_item->val;
            break;//Digital Ripple Enable/not
        case 10:
            ui_para.raderpic_scan_mode = select_item->val;
            break;//RaderPic scan mode
        case 11:
            ui_para.raderpic_scan_rate = select_item->val;
            break;//RaderPic scan rate
        case 12:
            ui_para.raderpic_move_rate = select_item->val;
...
void About_CallBack(uint8_t self_page_id, Option* select_item)
{
    switch(select_item->order) {
        //第0项是说明文字
        case 1:
            OLED_UIJumpToPage(self_page_id, &about_wououi_page);
            break;
        case 2:
            OLED_UIJumpToPage(self_page_id, &about_page_version_page);
            break;
        default:
            break;
    }
...
void TestUI_Init(void)
{
 ...
 //   HAL_Delay(100);
    ...
    //补充列表页面的初值
    setting_option_array[1].val = 70;
    setting_option_array[2].val = 25;
    setting_option_array[3].val = 0;
    setting_option_array[4].val = 0;
    setting_option_array[5].val = 0;
    setting_option_array[6].val = 0;
    setting_option_array[7].val = 66;
    setting_option_array[8].val = 88;
    setting_option_array[9].val = 0;//ui_para.digital_ripple;
