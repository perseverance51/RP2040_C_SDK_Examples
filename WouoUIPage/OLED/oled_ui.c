 ...
 * @return char* 转换后的字符串
 */
char * itoa_str(uint32_t num)
{
  static char str[11] = {0}; // 定义一个静态字符数组，用于存储转换后的字符串
  memset(str, 0, sizeof(str)); // 将字符数组清零
   snprintf(str, sizeof(str), "%lu", (unsigned long)num);
 // sprintf(str, "%" PRIu32, num); // 将无符号32位整数转换为字符串
 ...
 * @brief 显示标题页面
 *
 * 该函数用于显示标题页面，包括计算动画参数、绘制标题、标题指示器和图标等操作。
 *
 * @param page_addr 指向标题页面对象的指针
 */
static void OLED_TitlePageShow(PageAddr page_addr)
{
  ...
  x_point = ((OLED_WIDTH - TILE_INDI_W) - OLED_GetStrWidth(&(show_str[2]),TILE_B_TITLE_H)) / 2 + TILE_INDI_W;
  y_point = tp->title_y;
  //绘制title
  OLED_WinDrawStr(&w_all, x_point,y_point,TILE_B_TITLE_H,(uint8_t*)&(show_str[2]));
  //绘制title指示器

	// OLED_WinDrawStr(&w_all,0,y_point,TILE_B_TITLE_H,(uint8_t*)"->");
	// OLED_WinDrawStr(&w_all,OLED_WIDTH-TILE_B_TITLE_H,y_point,TILE_B_TITLE_H,(uint8_t*)"<-");
	//  OLED_WinDrawRBox(&w_all,0,TILE_ICON_S,tp->indi_x,TILE_INDI_H,0);// 当前菜单名字位置的所显示的内容。
	OLED_WinDrawBoxRightAngle(&w_all, 46, 0,36, 34, 6);//画矩形的四个直角
    OLED_WinDrawBoxRightAngle(&w_all, x_point-3, tp->title_y,OLED_GetStrWidth(&(show_str[2]),TILE_B_TITLE_H)+6, 24, 4);//画矩形的四个直角
  //绘制图标
...
static void OLED_ValWinShow(Option* win)
{
    ...
    OLED_WinDrawStr(&w_all, ui.vwin.l + 6 + OLED_GetStrWidth((char*)&(win->text[2]),WIN_FNOT_H),
                    (int16_t)ui.vwin.y+2,WIN_FNOT_H, (uint8_t*)(itoa_str(win->val))); //数值
}

// 封装回调函数调用逻辑
static void call_page_callback(Page* p, uint8_t page_id, Option* win) {
    if (p->cb != NULL) {
        p->cb(page_id, win);
    }
}
...
{
// 检查弹窗进场动画是否结束
const int is_animation_finished = (ui.cwin.y == ui.cwin.y_trg) && (ui.cwin.y != CON_WIN_Y_TRG);
...
static void OLED_ListDrawText_CheckBox(float start_y ,Option* item)
{
  ...
  {
      case '~': //数值弹窗的话 // 画行结尾的数值
        OLED_WinDrawStr(&w_all,CHECK_BOX_L_S, start_y + LIST_TEXT_S, LIST_TEXT_H,(uint8_t*)(itoa_str(item->val)));
        break;
      case '+': //二值选择框的话 //画结尾外框
...
void OLED_WavePageShow(PageAddr page_addr)
{
    ...
    if(wp->box_y == wp->box_y_trg && wp->y == wp->y_trg)//完成过度动画
    {
        ...
        //绘制文字
        temp_str = itoa_str(wp->option_array[wp->select_item].val);
...
static char* OLED_DigitalCreateText(DigitalPage * dp)
{
	...
	snprintf(str,sizeof(str), "%02d%c%02d%c%02d", dp->option_array[Digital_Pos_IndexLeft].val, dp->gap_char,
...
{
  ui_para.ani_param[TILE_ANI] = 80;  // 磁贴动画速度:120
  ui_para.ani_param[LIST_ANI] = 80;  // 列表动画速度:120
  ui_para.ani_param[WIN_ANI] = 80;   // 弹窗动画速度:120
//   ui_para.ani_param[SPOT_ANI] = 50;  // 聚光动画速度
//   ui_para.ani_param[FADE_ANI] = 30;  // 消失动画速度
  ui_para.ani_param[TAG_ANI] = 50;   // 标签动画速度,默认60
...
void OLED_UiInit(void)
{
    ...
    ui.vwin.l = (OLED_WIDTH - WIN_W)/2;//不管选框宽度，以字体宽度为准《/2》
...
void OLED_UIProc(void) //UI总任务(FSM)
{
    ...
    switch (ui.state) //ui状态机轮询
    {
        ...
        case ui_page_proc://因为主要的绘制图像都在这儿完成，所以进入前需清空一次buff
            OLED_ClearBuff();
//						OLED_WinDrawBoxRightAngle(&w_all, 46, 0,36, 35, 6);
