...
class RP2040_PWM
{
  ...
  bool setPWM_manual(const uint8_t& pin, uint16_t& level)
  {
    ...
    {
     printf("Error, not initialized for PWM pin = %d\n", _pin);
    ...
    {
      printf("Error, not correct PWM pin = %d", _pin);
   ...
   // printf("pin = ", _pin, ", PWM_CHAN =", pwm_gpio_to_channel(_pin));

    ////////////////////////////////

    _enabled = true;

    // printf("PWM enabled, slice =, _slice_num, , top =, _PWM_config.top,
    //              , div =%d", _PWM_config.div, ", level =", level);
